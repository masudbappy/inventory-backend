...
public class CustomerService {
    ...
    public Page<CustomerPaymentHistoryDto> getAllCustomersPaymentHistory(int page, int size, String sortBy,
        ...
        Pageable pageable = PageRequest.of(page, size,
                sortDir.equalsIgnoreCase("desc") ? Sort.by(actualSortBy).descending() : Sort.by(actualSortBy).ascending());

        // Always get both sales and payments
        List<CustomerPaymentHistoryDto> allHistory = new ArrayList<>();

        Page<CustomerPaymentHistoryDto> sales = customerRepository.findSaleHistory(customerId, searchQuery, null, PageRequest.of(0, Integer.MAX_VALUE));
        allHistory.addAll(sales.getContent());

        Page<CustomerPaymentHistoryDto> payments = customerRepository.findPaymentHistory(customerId, searchQuery, null, PageRequest.of(0, Integer.MAX_VALUE));
        allHistory.addAll(payments.getContent());

        // Filter by status if specified
        final String finalStatus = status;
        if (finalStatus != null && !"ALL".equalsIgnoreCase(finalStatus) && !"All".equalsIgnoreCase(finalStatus)) {
            allHistory = allHistory.stream()
                    .filter(item -> finalStatus.equalsIgnoreCase(item.getStatus()))
                    .collect(Collectors.toList());
        }
        ...
        List<CustomerPaymentHistoryDto> pageContent = totalElements > 0 ? allHistory.subList(start, end) : new ArrayList<>();
    ...
    private String mapSortField(String sortBy) {
        switch (sortBy) {
            case "date":
                return "paymentDate";  // PaymentLog uses 'paymentDate' field
            case "customerName":
